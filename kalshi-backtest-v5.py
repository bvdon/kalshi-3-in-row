"""
kalshi-backtest-v5.py — v4 + ATR Volatility Filter.

Same signal as v4 (multi-TF EMA), but adds:
  ATR FILTER: Skip if last candle's True Range > ATR_MULTIPLIER × ATR(ATR_PERIOD)
  High volatility = streak clustering. Skip it.
...
ATR_MULTIPLIER  = 1.5     # skip if last TR > ATR_MULTIPLIER × ATR
CONTRACTS       = 20       # contracts per trade (flat sizing)
...
def _atr(candles: list, period: int) -> list:
    """Returns ATR list aligned to candles (index i = ATR ending at candle i)."""
    trs = []
...
def main():
    ...
    # ── Pre-compute ATR ───────────────────────────────────────────────────────
    atr_all = _atr(all_candles, ATR_PERIOD)  # aligned to all_candles
    ...
    # ── Header ────────────────────────────────────────────────────────────────
    print(f"\n{'='*180}")
    print(f"  BTC 15m Backtest v5 — Multi-TF EMA + ATR Filter | {now_est}")
    print(f"  {len(candles):,} candles | {first_dt} → {last_dt}")
    ...
    print(f"  ATR SKIP: last TR > {ATR_MULTIPLIER}× ATR({ATR_PERIOD})")
    ...
    HEADER = (f"  {'DATE/TIME':<22} {'LAST':>12} {'OPEN':>12} {'CLOSE':>12} {'COLOR':<6} "
              ...
              f"{'SLOPE':>8} {'TR':>8} {'ATR':>8} {'SIDE':<5} {'SKIP?':<16} "
    ...
    for i, c in enumerate(candles):
        ...
        # ── ATR filter (use prev candle's TR vs ATR) ──────────────────────────
        prev_idx  = idx - 1
        prev_c    = all_candles[prev_idx]
        prev_atr  = atr_all[prev_idx]
        if prev_idx > 0:
            pp_close = all_candles[prev_idx - 1]["close"]
            prev_tr  = max(prev_c["high"] - prev_c["low"],
                           abs(prev_c["high"] - pp_close),
                           abs(prev_c["low"]  - pp_close))
        else:
            prev_tr = prev_c["high"] - prev_c["low"]
        ...
        def _row(side, skip_reason, win_str, profit_str, loss_str, net_str):
            ...
            atr_str = f"{prev_atr:>8.0f}"      if prev_atr  is not None else f"{'—':>8}"
            print(f"  {ts_str:<22} ${last_close:>11,.2f} ${candle_open:>11,.2f} ${candle_close:>11,.2f} {candle_color:<6} "
                  f"${ef:>10,.2f} ${es:>10,.2f} {e1h} "
                  f"{slope:>8.2f} {prev_tr:>8.0f} {atr_str} {side:<5} {skip_reason:<16} "
        ...
        # ── ATR filter ────────────────────────────────────────────────────────
        if prev_atr is not None and prev_tr > ATR_MULTIPLIER * prev_atr:
            skipped  += 1
            atr_skips += 1
            _row("—", "atr_spike", "—", "—", "—", "—")
            continue
    ...
    print(f"\n  {'─'*180}")
    print(f"  Trades: {total_trades:,}  |  Wins: {wins:,}  |  Losses: {losses:,}  |  Skipped: {skipped:,}  |  ATR Skips: {atr_skips:,}  |  Win Rate: {win_rate:.1f}%")
