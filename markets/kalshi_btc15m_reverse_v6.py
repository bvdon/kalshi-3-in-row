"""
markets/kalshi_btc15m_reverse_v6.py — BTC 15-minute Up/Down Reverse EMA + Anti-Martingale

Strategy (George Costanza + Anti-MG):
  ...
  - WIN → double contracts next trade (up to MAX_ANTI_ROUND)
...
Backtest results (2 years, 44k trades):
  Win rate: 51.9% | Max consec losses: 14 | Net P&L: +$6,541 @ anti-MG base 2
...
STRATEGY_NAME     = "REVERSE_EMA_V6"

EMA_FAST          = 21       # 15m EMA (slope + price filter)
EMA_SLOW          = 50       # 15m EMA (price filter)
EMA_1H            = 50       # 1H EMA (price filter)

BASE_CONTRACTS    = 20       # starting contracts (reset after every loss)
MAX_ANTI_ROUND    = 6        # max doublings: 20→40→80→160→320→640→1280
# Anti-MG ladder
...
STATE_PATH     = Path(__file__).parent.parent / "reverse_v6_state.json"
...
def _load_state() -> dict:
    if STATE_PATH.exists():
        try:
            ...
            pass
    default = {"anti_round": 0, "consecutive_wins": 0, "consecutive_losses": 0, "total_trades": 0}
    _save_state(default)
    return default
...
def _fast_resolve(trade: dict, state: dict) -> Optional[tuple]:
    ...
    try:
        ...
        if direction == side_bet:
            ...
            state["anti_round"]         = min(state.get("anti_round", 0) + 1, MAX_ANTI_ROUND)
            print(f"[{STRATEGY_NAME}] Fast resolve: WIN profit=${profit:.2f} | anti_round→{state['anti_round']}")
        else:
            _db_update_result(prev_ticker, profit=0.0, loss=total_cost)
            state["consecutive_wins"]   = 0
            state["consecutive_losses"] = state.get("consecutive_losses", 0) + 1
            state["anti_round"]         = 0
            print(f"[{STRATEGY_NAME}] Fast resolve: LOSS cost=${total_cost:.2f} | anti_round→0")
...
def _enter_new_cycle(state: dict, market_ticker: str, market: dict) -> None:
    ...
    anti_round = state.get("anti_round", 0)
    ...
    print(f"[{STRATEGY_NAME}] Signal: {signal or 'SKIP'}  anti_round={anti_round}  contracts={contracts}")
    print(f"{'='*60}")
